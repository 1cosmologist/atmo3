"""
Fetch uncoupled O2 line parameters from the HITRAN online database via the
HITRAN API (hapi) and save to a structured HDF5 file with full column
metadata. Uncoupled O2 lines are used (unlike the 60 GHz complex, which
requires the line-mixing coefficients handled separately) for simple
Gross/Van Vleck-Weisskopf lineshape absorption calculations.
...
Usage
-----
    python convert_o2_hitran_to_hdf5.py
"""
...
STEM        = "HITRAN_o2_uncoupled_0-1750GHz"
HEADER_FILE = DATA_DIR / "HITRAN_o2_uncoupled_0-1750GHz.header"  # generic HITRAN column layout
HDF5_FILE   = DATA_DIR / f"{STEM}.hdf5"
HAPI_CACHE  = DATA_DIR / "hapi_cache"

# HITRAN parameters for O2 isotopologue 1 (16O2)
MOL_ID  = 7
ISO_ID  = 1
NU_MIN  = 0.0     # cm⁻¹
NU_MAX  = 58.38   # cm⁻¹  ≈ 1750 GHz
TABLE   = "o2_hitran_fetch"
...
# ---------------------------------------------------------------------------
print(f"[ONLINE] Fetching O2 lines from HITRAN "
