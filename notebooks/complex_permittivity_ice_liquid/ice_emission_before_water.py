...
from .constants import ICE_DATA, LIQUID_WATER_HYBRID_COEFFS, LIQUID_WATER_HAMELIN_COEFFS
...
def complex_permittivity_ice(T, nu):
    ...
    Temperature T is in Kelvin, frequency nu is in GHz.
    """
    # 1. Real Part
    epsilon_prime = ICE_DATA['eps_inf'] + ICE_DATA['eps_t_coeff'] * (T - ICE_DATA['t_ref_real'])
    
    # 2. Imaginary Part
    theta = (ICE_DATA['theta_t_ref'] / T) - 1.0
    alpha = (ICE_DATA['alpha_0'] + ICE_DATA['alpha_1'] * theta) * jnp.exp(ICE_DATA['alpha_exp'] * theta)
    
    exp_bT = jnp.exp(ICE_DATA['beta_b'] / T)
    beta_M = (ICE_DATA['beta_b1'] / T) * (exp_bT / (exp_bT - 1.0)**2) + ICE_DATA['beta_b2'] * (nu**2)
    
    delta_beta = jnp.exp(ICE_DATA['delta_beta_0'] + ICE_DATA['delta_beta_1'] * (T - ICE_DATA['t_ref_triple']))
...
def eps_static_water_hamelin_from_K(T_K):
    ...
    The value for the static dielectric constant comes from Hamelin et al. 1998.
...
def complex_permittivity_liquid_water_hybrid_K(T_K, freq_GHz):
    ...
    epsilon : complex array
        Complex relative permittivity (epsilon' + i epsilon'')
    """
    ...
    # Delta_i(T) = a_i exp(-b_i T) with T in Celsius
    D1 = p["a1"] * jnp.exp(-p["b1"] * T)
    D2 = p["a2"] * jnp.exp(-p["b2"] * T)
    D3 = p["a3"] * jnp.exp(-p["b3"] * T)

    # tau_i(T) = c_i exp(d_i / (T + tc)) with T in Celsius
