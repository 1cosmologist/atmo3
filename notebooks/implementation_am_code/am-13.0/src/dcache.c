...
* am
*
* dcache.c                     S. Paine rev. 2021 January 25
*
...
static void         make_cache_file_pathstr(
        ...
        double T)
{
    ...
    for (slot = 1; slot <= CACHE_FILES_PER_HASHVAL; ++slot) {
        make_cache_file_pathstr(
                cache_filepath, cache_dirpath, requested_hdr.hashval, slot);
...
static char *dcache_path(void)
{
    ...
    if (pathptr != NULL) {
        if (pathptr[0] == '\0') { /* empty path string */
            pathptr = NULL;
        } else if (strlen(pathptr) >= AM_MAX_DIRPATH) {
            errlog(70, 0); /* path too long */
            pathptr = NULL;
        } else {
            strncpy(path, pathptr, (size_t)(AM_MAX_DIRPATH));
            if (check_for_dir_separator(path)) {
        ...
        char *cache_dirpath)
{
     ...
     */
    make_cache_file_pathstr(lockpath, cache_dirpath, new_hdr->hashval, 0);
     ...
     */
    for (slot = 1; slot <= CACHE_FILES_PER_HASHVAL; ++slot) {
        make_cache_file_pathstr(slotpath, cache_dirpath, new_hdr->hashval, slot);
     ...
     */
    make_cache_file_pathstr(
        slotpath, cache_dirpath, new_hdr->hashval, CACHE_FILES_PER_HASHVAL);
    if ((fp = fopen(slotpath, "rb")) != NULL) {
        fclose(fp);
    ...
    for (slot = CACHE_FILES_PER_HASHVAL; slot > 0; --slot) {
        make_cache_file_pathstr(
            old_slotpath, cache_dirpath, new_hdr->hashval, slot - 1);
        make_cache_file_pathstr(
            slotpath, cache_dirpath, new_hdr->hashval, slot);
        ...
        int old_slot)
{
     ...
     * promoting the file.
     */
    make_cache_file_pathstr(lockpath, cache_dirpath, hashval, 0);
    if ((fp = fopen(lockpath, "rb")) != NULL) {
        fclose(fp);
        return 1;
    }
     ...
     */
    make_cache_file_pathstr(old_slotpath, cache_dirpath, hashval, old_slot);
    ...
    for (slot = old_slot - 1; slot >= 0; --slot) {
        make_cache_file_pathstr(old_slotpath, cache_dirpath, hashval, slot);
        make_cache_file_pathstr(slotpath, cache_dirpath, hashval, slot + 1);
        if ((fp = fopen(old_slotpath, "rb")) != NULL) {
            fclose(fp);
            if (rename_with_retry(old_slotpath, slotpath)) {
                /*
