...
* am
*
* model.c                           S. Paine rev. 2023 May 5
*
 ...
 * ZTAN_ITER_MAX is the maximum number of iterations allowed
 * before convergence is deemed to have failed.
 */
...
*
*   Here, for spherical geometry, we use the approximations
*   from:
*
...
int compute_model(model_t *model, model_t *lmodel_in)
{
     ...
     * computation (i.e., if ngrid == 0).  This is done by
     * setting lmodel to NULL.  Otherwise, lmodel is just a copy
     * of lmodel_in.
     */
...
*   definition levels, n-1 is extrapolated in P/T, where T
*   is the temperature on the nearest model definition level
*   or layer.
*
        ...
        double *n, const model_t *model, const double z)
{
     ...
     * cause trouble since in such cases the source level is the
     * near side of the tangent point and no tangent level will
     * be inserted.  For this reason, we ignore the error return
     * unless P == 0.0.  (Note that a non-error return with
     * P == 0.0 is also possible.)
     */
    if (find_P_from_z(&P, model, z) && P == 0.0)
        return 1;
    /*
     * Scan for two model definition layers having zbase values
     * that bracket z.
     */
    lnum_hi = -1;
    for (lnum_lo = 0; lnum_lo < model->nlayers; ++lnum_lo) {
        layer_t *layer = model->layer[lnum_lo];
        if (layer->type != LAYER_TYPE_DEF || layer->Pbase < model->Ptoa)
            continue;
        else if (layer->h >= 0.0 && layer->dP == 0.0)
            continue; /* exclude plume layers */
        else if (layer->zbase > z)
            lnum_hi = lnum_lo;
        else if (layer->zbase <= z)
            break;
    }
    if (lnum_lo == model->nlayers) {
        /*
         * There is no model definition level with zbase below z.
         * n will be extrapolated below the layer stack.
         */
        layer_ref = model->layer[lnum_hi];
    } else if (lnum_hi < 0) {
        /*
         * There is no model definition level with zbase above z.
         * n will be extrapolated above the top model
         * definition layer having Pbase > 0.
         */
        layer_ref = model->layer[lnum_lo];
    } else {
        /*
         * Bracketed in z.  n will be computed by linearly
         * interpolating in P / T
         */
        layer_t *layer_lo = model->layer[lnum_lo];
        layer_t *layer_hi = model->layer[lnum_hi];
        double T;
    ...
    /*
     * Extrapolation outside model definition levels is
     * isothermal, so n-1 is simply scaled by P.
     */
    if (layer_ref->Pbase > 0.0) {
        *n  = layer_ref->nbase - 1.0;
        *n *= P / layer_ref->Pbase;
        *n += 1.0;
    } else {
        *n  = 1.0;
        ...
        double *P, const model_t *model, const double z)
{
    ...
    /*
     * Scan for two model definition layers below the
     * top-of-atmosphere level that have zbase values that
     * bracket z.
     */
    lnum_hi = -1;
    for (lnum_lo = 0; lnum_lo < model->nlayers; ++lnum_lo) {
    ...
    if (lnum_lo == model->nlayers) {
        /*
         * There is no model definition level with zbase below z.
         * P will be computed by isothermal extrapolation from
         * above.
         */
        layer_ref = model->layer[lnum_hi];
    } else if (lnum_hi < 0) {
        ...
        /*
         * Bracketed in z.  P will be computed by linearly
         * interpolating log(P) in z.
         */
        layer_t *layer_lo = model->layer[lnum_lo];
        layer_t *layer_hi = model->layer[lnum_hi];
        *P = log_y_interp(
    ...
    /*
     * Isothermal extrapolation outside model definition layer
     * base levels
     */
    T = model->PTmode & PTMODE_TBASE ? layer_ref->Tbase : layer_ref->T;
    if (layer_ref->Pbase == 0.0) {
        *P = 0.0;
        return 0;
    } else if (T == 0.0) {
        ...
        *P = 0.0;
        ...
        x  = (layer_ref->zbase - z) * layer_ref->gbase / (R * T);
        if (x >= log(model->Pmax / layer_ref->Pbase)) {
            *P = model->Pmax;
            return 1;
        } else {
            *P = layer_ref->Pbase * exp(x);
            if (*P < model->Ptoa)
                *P = 0.0;
        }
    }
        ...
        double *z, const model_t *model, const double P)
{
    ...
    if (P <= 0.0) {
        ...
        /*
         * Warn only if P is more than two scale heights
         * above the highest model definition level.
         */
        errlog(164, 0);
    ...
    /*
     * Scan for two model definition layers having Pbase values
     * that bracket P.
     */
    lnum_hi = -1;
    for (lnum_lo = 0; lnum_lo < model->nlayers; ++lnum_lo) {
    ...
    if (lnum_lo == model->nlayers) {
        /*
         * There is no model definition level with Pbase > P.
         * z will be computed by isothermal extrapolation from
         * above.
         */
        layer_ref = model->layer[lnum_hi];
    } else if (lnum_hi < 0 || model->layer[lnum_hi]->Pbase <= 0.0) {
        /*
         * There is no model definition level with Pbase < P
         * and Pbase > 0.  z will be computed by isothermal
         * extrapolation from below.
         */
        layer_ref = model->layer[lnum_lo];
    } else {
        /*
         * Bracketed in P.  z will be computed by linearly
         * interpolating z in log(P).
         */
        layer_t *layer_lo = model->layer[lnum_lo];
        layer_t *layer_hi = model->layer[lnum_hi];
        *z = log_x_interp(
    ...
    R *= unit_tab[UNIT_M].factor * unit_tab[UNIT_M].factor;
    /*
     * Note that layer_ref->P cannot be zero, since
     * model->Pmax would then also be zero.  The checks
     * above ensure that P > 0 and P < model->Pmax.
     */
    *z = layer_ref->zbase -
        (R * T / layer_ref->gbase) * log(P / layer_ref->Pbase);
    return 0;
}   /* find_z_from_P() */
...
static int set_propagation_path_indices(model_t *model)
{
     ...
     * levels below the base level of the lowest defined model
     * layer might be OK, but log a warning to inform the user.
     */
...
int setup_atmospheric_model(model_t *model, model_t *lmodel)
{
    ...
    /*
     * The layer definitions in the original model configuration
     * define the atmospheric model.  In hydrostatic models,
     * additional interpolated or extrapolated levels may be
     * added for source, observing, or tangent levels.  These
     * require the addition of new model layers created by
     * splitting a model definition layer (parent layer) into
     * parts, or by extrapolating below the layer stack.
     *
