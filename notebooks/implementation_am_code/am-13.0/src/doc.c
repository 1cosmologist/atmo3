...
* am
*
* doc.c                          S. Paine rev. 2023 July 17
*
...
/*
 * The following string contains the URL for the complete manual.
 * Current recommended practice that DOIs be displayed as stable
 * links in the form https://doi.org/[DOI].
 */
static const char AM_DOC_URL[] = "https://doi.org/10.5281/zenodo.640645";


/*
 * Lengthy built-in documentation strings are organized as string
 * arrays of single lines.  This avoids going over the old 509
 * char minimum upper limit on string literals (increased to 4096
 * chars in C99), is reasonably easy to edit, and eliminates
 * embedded newline chars.
 */
...
static const char* reference_list[] = {
...
static void print_doc_strings(FILE*, const char**, const int);

/***********************************************************
* void legal(void)
*
* Purpose:
*   prints a legal notice to stdout
***********************************************************/

void legal(void) {
    version(stdout);
    print_doc_strings(
        stdout, legal_notice, sizeof(legal_notice) / sizeof(char*));
    return;
} /* legal() */
...
*
* Purpose:
*   prints a list of references to stdout
***********************************************************/
...
void references(void) {
    ...
    printf(
      ...
      "(%s).\n\n\n",
      AM_DOC_YEAR, AM_DOC_VERSION_MAJOR, AM_DOC_VERSION_MINOR, AM_DOC_URL);
    print_doc_strings(
        stdout, reference_list, sizeof(reference_list) / sizeof(char*));
    return;
} /* references() */

/***********************************************************
* void print_doc_strings(FILE *stream, const char *str[], const int n)
*
* Purpose:
*   prints an array str[] composed of n one-line strings
*   to a stream, adding a newline character to the end of
*   each printed string.
***********************************************************/

static void print_doc_strings(FILE *stream, const char *str[], const int n)
{
    int i;
    for (i = 0; i < n; ++i)
        fprintf(stream, "%s\n", str[i]);
...
*
* Purpose:
*   Prints a usage message to a stream.
*
* Arguments:
*   none
************************************************************/
...
{
    fprintf(stream,
        "Usage: %s [option] FILE [param1 param2...]\n"
        "\n"
