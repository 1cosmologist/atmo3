...
* am
*
* fit.c                         S. Paine rev. 2023 August 22
*
...
char *fit_data_delimiters(const char *fmt)
{
    ...
    if (fmt != NULL) {
        ...
        /*
         * Newline and carriage return are always included in the
         * delimiter set.
         */
        strncpy(fit_data_delimstr, fmt, (size_t)(FIT_MAX_DELIMSTRSIZE - 3));
        strcat(fit_data_delimstr, "\n\r");
    }
...
char *fit_data_format(const char *fmt)
{
    ...
    if (fmt != NULL) {
        ...
        strncpy(fit_data_fmt, fmt, (size_t)(FIT_MAX_FMTSTRSIZE - 1));
    }
...
static char *fit_indir(void)
{
    ...
    if ((envstr = getenv(FIT_INPUT_PATH_ENV_STRING)) != NULL) {
        if (strlen(envstr) < AM_MAX_DIRPATH) {
            strncpy(indir, envstr, (size_t)(AM_MAX_DIRPATH - 1));
...
static char *fit_outdir(void)
{
    ...
    if ((envstr = getenv(FIT_OUTPUT_PATH_ENV_STRING)) != NULL) {
        if (strlen(envstr) < AM_MAX_DIRPATH) {
            strncpy(outdir, envstr, (size_t)(AM_MAX_DIRPATH - 1));
