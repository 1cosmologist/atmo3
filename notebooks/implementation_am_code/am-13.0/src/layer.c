...
* am
*
* layer.c                      S. Paine rev. 2022 December 2
*
...
*   layer lnum.  For a layer associated with an interpolated
*   level this is first model definition layer with lnum >=
*   clnum.  (Note that this means that a model definition
*   layer is its own parent layer.)  For layer associated
*   with a level extrapolated below the model definition
*   layers, the parent layer is the lowest model definition
*   layer.
*
* Arguments:
...
*
*   For each of these layer types, only one inserted layer
*   of a given type is allowed.  If an existing layer of the
*   same type exists, and is already in the correct position
*   in the layer stack, then zbase and Pbase are simply
*   updated (on model, but not lmodel).  If an existing
*   layer is found but it is out of position, it is deleted;
*   a new layer is then inserted at the correct position (on
*   both model and lmodel).
*
            ...
            if (layer->type == LAYER_TYPE_DEF) {
                if (layer->Pbase <= 0.0) {
                    ...
                    if (model->PTmode & PTMODE_TBASE) {
                        /*
                         * Tbar is the harmonic mean temperature
                         * between T_ref and T.
                         */
                        double T_ref = layer_ref->Tbase;
                        double T     = layer->Tbase;
                        Tbar = 2.0 * T_ref * T / (T_ref + T);
                    } else {
                        /*
                    ...
                    dz = (log(P_ref) - log(P)) * R * Tbar / layer_ref->gbase;
...
*   base temperature, this function sets an interpolated
*   midpoint temperatures.  Interpolation is linear in
*   log(P) for hydrostatic models, and simply linear
*   otherwise.
*
*   When model definition layer temperatures are set by
*   midoint temperature, layers are assumed isothermal.  In
*   that case, base temperatures are set to match the
*   midpoint temperature.  This convention avoids the need
*   to check the PTmode in some cases.
*
...
int set_interpolated_layer_variables(model_t *model)
{
    ...
    if (model->PTmode & PTMODE_TBASE) {
        layer_t *pplayer;
        /*
         * In PTMODE_TBASE, temperatures are interpolated in
         * log(P) between model definition base levels, and
         * extrapolated isothermally outside these levels.
         */
