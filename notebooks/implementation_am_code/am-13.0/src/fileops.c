...
* am
*
* fileops.c                     S. Paine rev. 2017 January 2
*
...
/*
 * Initial retry delays for failed file operations.  Retry
 * delays double on each attempt, up to the maximum number
 * of retries.
 */
 ...
 * '/' - Unix, POSIX, OS X.  Also works in standard C library under Windows.
 * '\\' - Windows native
 * ':' - Older MacOS
 * ']' - VMS
 */
...
*
* Purpose:
*   Creates a temporary file, opened in binary update mode, and
*   returns a file pointer.  The file is created in the directory
*   named by the string dirpath.  If dirpath is NULL, or points
*   to an empty string, the file is created in the current
*   directory.  The full path to the temporary file is written
*   to the buffer fpath, which should be at least AM_MAX_PATH
*   characters long.
*
*   If the file cannot be created, NULL is returned, and an empty
*   string is written to fpath.
*
*   The last character of dirpath must be a directory separator
*   appropriate for the current environment.
*
* Arguments:
*   const char *path - path to directory
*   char *fpath - pointer to buffer to receive path to file.
*
* Return:
...
FILE *am_tmpfile(const char *dirpath, char *fpath)
{
    ...
    for (i = 0; i < TMPFILE_NUM_TRIES; ++i) {
         ...
         */
        sprintf(fpath, TMPNAME_FMT, ((dirpath == NULL) ? "" : dirpath), rand());
        if ((stream = fopen(fpath, "rb")) != NULL) {
            ...
            /*
             * Didn't exist already, so try opening in binary update mode.
             * If, by chance, this process lost a race for this file name,
             * the outcome depends on the implementation of fopen().  If
             * "wb+" opens with exclusive access, then this fopen() fails
             * and all is well.  However, another possibility is that this
             * fopen() causes the other process to lose its file name to
             * file pointer association.  In am, a possible consequence of
             * this would be a file getting copied to the wrong hash bucket
             * of the disk cache. This situation resolves itself-- the
             * misplaced file will always be a cache miss based on its header
             * data, and it will eventually be evicted from the cache.
             */
...
*
* Purpose:
*   Checks that dirpath ends in a directory separator character.
*   If not, a default one is appended, unless the resulting
*   string would be longer than AM_MAX_DIRPATH.
*
* Arguments:
...
int check_for_dir_separator(char *dirpath)
{
    ...
    if (strchr(DIR_SEPARATORS, dirpath[length - 1]) == NULL) {
        if (length + sizeof(DEFAULT_DIR_SEPARATOR) < AM_MAX_DIRPATH) {
            strcat(dirpath, DEFAULT_DIR_SEPARATOR);
        } else {
            return 1;
        }
    }
