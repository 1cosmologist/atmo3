...
* am
*
* output.c                         S. Paine rev. 2023 May 15
*
...
static void write_output_headers(FILE*, simplex_t*);
...
*
* Purpose:
*   Writes the model configuration data to a stream, in canonical
*   config file format.
*
        ...
        simplex_t *simplex)
{
    ...
    /*
     * Print version, run timing, cache statistics, and fit convergence
     * information.
     */
    ...
    if (output[ALL_OUTPUTS].flags & OUTPUT_FITTED) {
        ...
        if (fit_data->w_col == 0) {
            /*
             * No weights were defined, so report standard deviation
             */
            ...
            /*
             * Weights were defined, so report reduced chi-squared
             */
            fprintf(stream,
    ...
    fprintf(stream, "\n");
    /*
     * If this was a fit, print fit configuration and kcache information
     */
    if (output[ALL_OUTPUTS].flags & OUTPUT_FITTED) {
        fprintf(stream,
                "# fit %s %s\n",
    ...
    for (lnum = 0; lnum < model->nlayers; ++lnum) {
        ...
        if (layer->type != LAYER_TYPE_DEF) {
            ...
            /*
             * This is parent layer for extrapolated layer(s)
        ...
        const int  verbosity)
{
    ...
    if (column->N_mode & N_BY_VMR && !(column->vmr_stat & VMR_BY_RH)) {
        /*
        ...
        /*
         * For column density by RH, report both vmr and column
         * density as comments.  If the RH value is offset or
         * scaled, show the adjusted RH value in front of the
         * vmr, since the vmr is computed from the adjusted RH.
         */
        fprintf(stream,
                "%s %g%%",
        ...
        simplex_t *simplex)
{
    int j;
    int freq_dig;
    double *f; /* output frequency grid (model or IF) */
    double x;
    gridsize_t nout; /* number of output grid points */
    gridsize_t i, imin, imax;

    f = output[OUTPUT_FREQUENCY].spectrum;
    nout = (model->ifmode ? model->nif : model->ngrid);
    /*
    ...
    if (model->headers)
        write_output_headers(stream, simplex);
    /*
     * Write the spectra.
     */
    for (i = imin; i <= imax; ++i) {
        for (j = 0; outcol[j] != 0; ++j) {
            ...
            fprintf(stream, (j == 0) ? "%.*e" : " %.*e",
                    outcol[j] == OUTPUT_FREQUENCY ? freq_dig: FLT_DIG, x);
...
/***********************************************************
* static void write_output_headers(
*   FILE *stream,
*   simplex_t *simplex)
*
* Purpose:
    ...
    for (j = 0; outcol[j] != 0; ++j) {
        ...
        if (output[outcol[j]].flags & OUTPUT_JACOBIAN) {
            ...
            for (i = 0; i < simplex->n; ++i) {
                ...
                if (output[ALL_OUTPUTS].flags & OUTPUT_JACOBIAN_ERRS)
                    fprintf(stream, " round_err trunc_err");
            }
        }
    }
    fprintf(stream, "\n");
...
*
* Purpose:
*   Writes the PTmode as text string to a stream, followed
