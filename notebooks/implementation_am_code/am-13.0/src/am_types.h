...
* am
*
* am_types.h                        S. Paine rev. 2023 May 5
*
...
struct model_t {
    ...
    double g;               /* gravitational acceleration at R0 [m s^-2]    */
    ...
    double Ptoa;            /* top of atmosphere in hydrostatic models      */
    double Pmax;            /* maximum extrapolated P in hydrostatic models */
    ...
    double *tau_fsl;        /* natural log of free space loss               */
    double *k_out;          /* spectral absorption coefficient for k output */
    ...
    int path_min;           /* minimum of path_begin and path_end           */
...
struct layer_t {
    ...
    double dP_def;      /* dP on model definition layer                     */
    ...
    double gbase;       /* gravitational acceleration at zbase              */
    double nbase;       /* base level refract. index in hydrostatic models  */
    ...
    int type;           /* layer type - see enum in layer.h                 */
 ...
 * column density.
 *
 * When hydrostatic layers are split by interpolated levels, and there
 * are columns with explicitly-defined column density, the column
 * density is distributed across the layers in proportion to their
 * respective pressure drops dP.  N_def holds the defining value of N
 * on the original parent layer, and N is the column density on each
 * interpolated layer.
 *
...
struct column_t {
    ...
    double N_def;       /* N on model definition layer                      */
