...
* am
*
* transform.c                  S. Paine rev. 2021 October 30
*
*
...
/*
 * This file contains functions for fast Fourier, Hartley, and
 * Hilbert transforms, written with the aid of Joerg Arndt's
 * excellent text, "Matters Computational," online at
 * http://www.jjj.de/fxt.  The FFT's and FHT's here omit the
 * usual reverse-binary permutations, since the transform-domain
 * operations in am (convolution and Hilbert transformation) can
 * be done on the permuted data.
 *
 ...
 * 
 * For testing, there are two reverse binary permutation
 * functions here, for complex and real arrays:
 *
 ...
 *
 * The FFT and FHT functions use a combined recursive-iterative
 * strategy, described in
 * 
 *   Singleton, R. C. 1967. "On Computing the Fast Fourier
 *   Transform," Communications of the ACM 10:647.
 *
 * See also
 *
 *   M. Frigo and S. G. Johnson 2005. "The Design and
 *   Implementation of FFTW3," Proc.  IEEE 93:216.
 *
 * Taking fft_dif() as an example, fft_dif() calls the recursive
 * FFT fft_dif_rec(), setting an initial branch count of 1 for
 * the recursion.  fft_dif_rec() calls itself recursively (twice)
 * to perform two half-sized transforms, until the transform is
 * small enough to be done entirely in L1 cache, at which point
 * it calls an iterative FFT, fft_dif_iter(), instead.  The
 * branch count is doubled at each recursion stage.  Under
 * OpenMP, if the environment variable OMP_NESTED=true, each
 * recursive call starts two new threads until the branch count
 * exceeds the maximum number of threads for a parallel region.
 *
 * There are two versions of each of the iterative FFT's and
 * FHT's: a conventional one (e.g. fft_dif_iter()) which
 * minimizes trigonometric computations at the expense of
 * non-local memory access; and another (e.g. fft_dif_iter_seq())
 * which accesses memory with unit stride.  As Arndt points out
 * in the text referenced above, on modern hardware the
 * unit-stride version can be significantly faster, despite the
 * larger number of arithmetic operations.  Tests on AMD Opteron
 * and Intel Pentium 4 showed this to be true down to transforms
 * fitting in L2 cache, but for transforms fitting in L1 cache
 * the non-local transforms were faster.  Overall, the fastest
 * strategy is to do recursive computation down to the L1 cache
 * size, then switch over to the non-local iterative transform.
 */
...
*   
* Arguments:
*   double *x       - array of 2n doubles, representing n
*                     real numbers
*   unsigned long n - dimension of x, must be a power of 2
************************************************************/
