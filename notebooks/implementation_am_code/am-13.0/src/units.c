...
* am
*
* units.c                          S. Paine rev. 2023 May 15
*
...
/***********************************************************
* int convert_unit(double *q, const char *unit)
*
* Purpose:
*   Converts from user units to am native units, modifying
*   the stored value of the quantity q.
*
* Arguments:
*   double *q - pointer to quantity to be converted
*   const char *unit - name of unit to be converted from
*
* Return:
*   0 if OK
*   1 on error
...
int convert_unit(double *q, const char *unit)
{
    ...
    for (i = 0; i < UNIT_END_OF_TABLE; ++i) {
        if (strcmp(unit_tab[i].name, unit) == 0) {
            *q += unit_tab[i].offset;
            *q *= unit_tab[i].factor;
            return 0;
        }
...
/***********************************************************
* int convert_unit_differential(double *q, const char *unit)
*
* Purpose:
*   Converts from user units to am native units, modifying
*   the stored value of the quantity q.  Any offset in the
*   unit conversion, as for degrees C, is not included, as
*   the converted value is meant to be a differential
*   quantity.
*
* Arguments:
*   double *q - pointer to quantity to be converted
...
/***********************************************************
* int sprint_with_unit(char *s, const char *fmt, double q, int unitnum)
*
* Purpose:
*   Prints a dimensioned quantity q to a string s, using a
*   specified format and unit.  The quantity q, assumed
*   to be stored in am native units, is converted to the specified
*   unit for printing, but the stored value is not modified.
*
* Arguments:
