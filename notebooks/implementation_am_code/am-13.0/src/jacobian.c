...
* am
*
* jacobian.c                    S. Paine rev. 2023 August 22
*
        ...
        simplex_t *simplex)
{
    ...
    for (j = 0; j < simplex->n; ++j) {
         ...
         *
         * Note that there are potentially two variable
         * unmappings involved here: first, simplex vertices are
         * stored by default as the log of the corresponding
         * coordinate in simplex space, and second, some model
         * variables (vmr's and signed quantities) are mapped
         * from the physical domain to [0, inf].
         *
         * The step size is computed from the characteristic
         * scale supplied by the user.
         */
        v0var = simplex->vertex[0][j];
        mvar  = simplex->logarithmic ? exp(v0var) : v0var; 
