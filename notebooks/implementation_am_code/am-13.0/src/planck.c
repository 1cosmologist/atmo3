...
* am
*
* planck.c                      S. Paine rev. 2019 January 9
*
* Planck functions.
************************************************************/
...
void planck_benchmarks(void)
{
    ...
    const double BMARK_EST_TIME = 0.1;  /* [s] */
    const double BMARK_RUN_TIME = 0.5;  /* [s] */
    double t, tstart;
    double *f = NULL;
    double *B = NULL;
    double *T = NULL;
    ...
    for (ngrid = BMARK_MIN_NGRID;
        ...
        for (j = 0; j < ncalls; ++j)
            B_Planck(B, f, BMARK_DF, ngrid, BMARK_T);
        t = am_timer(tstart);
        printf("%20s, n = %ld, %d call%s: %7.1f ns per point\n",
            "B_Planck()",
            (long int)ngrid,
            ncalls,
            ncalls == 1 ? "" : "s",
     ...
     * guide to parallelizing those loops.
     */
    for (ngrid = BMARK_MIN_NGRID;
        ngrid <= BMARK_MAX_NGRID; ngrid *= BMARK_NGRID_GROW) {
        tstart = am_timer(0.0);
        ...
        for (j = 0; j < ncalls; ++j)
            ...
            for(i = 0; i < ngrid; ++i)
                T[i] = T_Planck(f[i], B[i]);
        t = am_timer(tstart);
        printf("%20s, n = %ld, %d * n call%s: %7.1f ns per call\n",
            "T_Planck()",
            (long int)ngrid,
            ncalls,
            (ncalls * ngrid) == 1 ? "" : "s",
