...
* am
*
* config.c                         S. Paine rev. 2023 May 15
*
* model configuration
************************************************************/
...
static int kwfunc_headers(char*[], const int, model_t*);
    ...
    while (fgets(buf, (int)sizeof(buf), cfgfile) != NULL) {
        ...
        switch (get_keyword_num(tok[0])) {
            ...
            case KW_H:
                ...
                break;
            case KW_HEADERS:
                status = kwfunc_headers(tok, ntok, model);
                break;
            case KW_IFSPEC:
    ...
    switch (get_keyword_num(tok[0])) {
        ...
        case KW_LAYER:
            ...
            for (i = 0; i < model->nlayers; ++i) {
                if (model->layer[lnum]->type == LAYER_TYPE_DEF)
                    ++n_def_layers;
            }
...
*     and are replaced with argv[2], argv[3],...
...
static void parse_error(const char* format, ...)
{
    ...
    indent = sprintf(buf, LINE_ID_FMT, CONFIG_FILE_NAME, CONFIG_FILE_LINENUM);
    if (format[0] == '\t') {
        ...
        simplex_t *simplex)
{
    ...
    if (!ci_strcmp(tok[2], "hydrostatic") || 
         ...
         * interval [0, 1] to [0, 1 / DBL_EPSILON].  The mapping is
         * xvmr = vmr / (1 + DBL_EPSILON + vmr).
         */
        ...
        int lnum)
{
    ...
    if (model->PTmode & PTMODE_P_NOT_SET) {
        model->PTmode ^= PTMODE_P_NOT_SET;
    ...
    if (ntok == 5 && simplex != NULL) {
        ...
        sprintf(VARNAME, "dP (layer %d)", lnum);
        if (add_var_to_simplex(
                    simplex,
                    VARNAME,
                    &(model->layer[lnum]->dP_def),
        ...
        int lnum)
{
    ...
    if (ntok == 5 && simplex != NULL) {
        ...
        sprintf(VARNAME, "h (layer %d)", lnum);
        if (add_var_to_simplex(
                    simplex,
                    VARNAME,
                    &(model->layer[lnum]->h),
...
*
* keyword:
*   header - print output column headers
*
* syntax:
*   headers 0|1
************************************************************/
        ...
        model_t *model)
{
    ...
    if (get_bool_val(&(model->headers), tok[1]))
        return KWFUNC_SYNTAX_ERROR_CONTINUE;
    return KWFUNC_SUCCESS;
} /* kwfunc_headers() */
                ...
                for (knum = 0; knum < K_TYPE_END_OF_TABLE; ++knum) {
                    if (k_type[knum].dep_flags & DEP_ON_LSHAPE) {
                        ...
                        if (nchar == 0)
                            ...
                            nchar += fprintf(stderr, "  ");
                        nchar += fprintf(stderr, "%s", k_type[knum].name);
                    }
                }
                fprintf(stderr, "\n");
                return KWFUNC_SYNTAX_ERROR_CONTINUE;
        ...
        const int ntok)
{
    ...
    int status;
    if (ntok < 2) /* no arrays specified - keep defaults */
        return KWFUNC_SUCCESS;
    itok = 1;
    /*
    ...
    while (itok < ntok) {
        ...
        if (i == OUTPUT_END_OF_TABLE) {
            parse_error("Unrecognized array name or unit (%s?)\n", tok[itok]);
            status = KWFUNC_SYNTAX_ERROR_CONTINUE;
        ...
        int lnum)
{
    ...
    if (ntok == 5 && simplex != NULL) {
        ...
        sprintf(VARNAME, "P (layer %d)", lnum);
        if (add_var_to_simplex(
                    simplex,
                    VARNAME,
                    &(model->layer[lnum]->P),
...
*
* syntax:
*   PTmode midpoint | ((P | Pbase | dP) [T | Tbase])
************************************************************/
...
*
* keyword:
*   simplex_log - use logarithmic mapping of simplex coordinates
*
* syntax:
*   simplex_log 0|1
************************************************************/
        ...
        int lnum)
{
    ...
    if (model->PTmode & PTMODE_T_NOT_SET) {
        model->PTmode ^= PTMODE_T_NOT_SET;
        model->PTmode |= PTMODE_T;
    }
    ...
    if (ntok == 5 && simplex != NULL) {
        ...
        sprintf(VARNAME, "T (layer %d)", lnum);
        if (add_var_to_simplex(
                    simplex,
                    VARNAME,
                    &(model->layer[lnum]->T),
        ...
        int lnum)
{
    ...
    if (ntok == 5 && simplex != NULL) {
        ...
        sprintf(VARNAME, "Tbase (layer %d)", lnum);
        if (add_var_to_simplex(
                    simplex,
                    VARNAME,
                    &(model->layer[lnum]->Tbase),
