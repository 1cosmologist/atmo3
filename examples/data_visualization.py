...
plt.imshow(
    ...
    cmap=cmo.cm.dense, origin='lower', vmin=atmo.properties['precipitable water vapor']['value']['f'].min(), vmax=atmo.properties['precipitable water vapor']['value']['f'].max()
)
...
print(f"Mean PWV: {mean_pwv:.4f} mm, Std PWV: {std_pwv:.4f} mm")

# np.savez_compressed(f'./examples/atmo_fields_UTC{UTC_hour}.npz',
#     water_vapor_density=np.asarray(atmo.components['water vapor density'].field), 
#     precipitable_water_vapor=np.asarray(atmo.properties['precipitable water vapor']['value']['f']))
# # Create a 3D figure
# fig = plt.figure(figsize=(10, 8))
# ax = fig.add_subplot(111, projection='3d')

# # Create a 3D grid of points
# x, y, z = np.indices(atmo.components['water vapor density'].field.shape)

# # Plot the cube with transparency
# ax.voxels(atmo.components['water vapor density'].field, edgecolor=None, facecolors=cmo.cm.deep(atmo.components['water vapor density'].field), alpha=0.25)

# # Set axis limits
# ax.set_xlim(0, box_length_in_m)
# ax.set_ylim(0, box_length_in_m)
# ax.set_zlim(site_altitude, site_altitude + box_length_in_m)
...
# # Show the plot
# fig.savefig(f'./examples/water_vapor_density_3d_UTC{UTC_hour}.png')
