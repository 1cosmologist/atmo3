...
P_surface = 58800.0  # Surface pressure in Pa or kg/(m.s^2)

elevation = 45.0  # degrees
azimuth = 30.0  # degrees
pol_psi = 0.0  # degrees

pointing = jax.numpy.array([elevation, azimuth, pol_psi])

detector_position = jax.numpy.array(
    [0, 0, 0]
)  # Detector position in meters at ground level

print(
    f"Initializing atmosphere with nside_grid={nside_grid} and box_length_in_m={box_length_in_m} and site_altitude={site_altitude} meters"
)
atmo = a3.Atmosphere(
...
pofk_array = (k0**2.0 + k_array**2) ** -(11 / 6)
...
mean_q_profile = np.load("./examples/mean_q_profiles_2023_07.npz")[
    "typical_profiles"
]  # kg/kg
delta_q_profile = np.load("./examples/delta_q_profiles_2023_07.npz")[
    "delta_q_map"
]  # kg/kg
mean_ta_profile = np.load("./examples/mean_ta_profiles_2023_07.npz")[
    "typical_profiles"
]  # K
...
atmo.add_property(
    property_name="temperature", property_unit="K", property_value=rescale_mean_t
)
...
plt.imshow(
    ...
    cmap=cmo.cm.rain,
    origin="lower",
    vmin=atmo.components["specific humidity"].field.min(),
    vmax=atmo.components["specific humidity"].field.max(),
)
...
plt.ylabel("z (m)")
plt.savefig(
    f"./examples/specific_humidity_yz_plane_wrescaling_UTC{UTC_hour}.png",
    bbox_inches="tight",
)
plt.close()
...
plt.imshow(
    ...
    cmap=cmo.cm.thermal,
    origin="lower",
    vmin=atmo.components["virtual temperature"].field.min(),
    vmax=atmo.components["virtual temperature"].field.max(),
)
...
plt.ylabel("z (m)")
plt.savefig(
    f"./examples/virtual_temperature_yz_plane_UTC{UTC_hour}.png", bbox_inches="tight"
)
plt.close()
...
plt.imshow(
    ...
    cmap=cmo.cm.dense,
    origin="lower",
    vmin=atmo.components["pressure"].field.min(),
    vmax=atmo.components["pressure"].field.max(),
)
...
plt.imshow(
    ...
    cmap=cmo.cm.ice_r,
    origin="lower",
    vmin=atmo.components["water vapor density"].field.min(),
    vmax=atmo.components["water vapor density"].field.max(),
)
...
plt.ylabel("z (m)")
plt.savefig(
    f"./examples/water_vapor_density_yz_plane_UTC{UTC_hour}.png", bbox_inches="tight"
)
plt.close()

# atmo.compute_pwv_along_z()
...
#     cmap=cmo.cm.deep,
#     origin="lower",
#     vmin=atmo.properties["precipitable water vapor"]["value"]["f"].min(),
#     vmax=atmo.properties["precipitable water vapor"]["value"]["f"].max(),
# )
# plt.colorbar(label="mm")
# plt.title(
#     f"Precipitable water vapor field y-z plane at z={site_altitude} m, UTC hour {UTC_hour}"
# )
# plt.xlabel("x (m)")
# plt.ylabel("y (m)")
# plt.savefig(
#     f"./examples/precipitable_water_vapor_yz_plane_UTC{UTC_hour}.png",
#     bbox_inches="tight",
# )
# plt.close()

atmo.compute_pwv_along_los(detector_position=detector_position, pointing=pointing)

print(atmo.properties["precipitable water vapor"]["value"]["f"].shape)
print(type(atmo.properties["precipitable water vapor"]["value"]["f"]))
print(atmo.properties["precipitable water vapor"]["value"]["f"])

## the single PWV value of the LOS integral, for a given position and pointing.
